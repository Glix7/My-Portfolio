...
const PrivacyPolicy: React.FC = () => {
  ...
  useEffect(() => {
    const fetchDoc = async () => {
      try {
        // Fetch by slug "privacy-policy" from 'legals' collection
        const record = await pb.collection('legals').getFirstListItem<LegalDoc>('slug="privacy-policy"');
        setDoc(record);
      } catch (error) {
  ...
  // Fallback content if DB record is missing (Rich Text Mock)
  const content = doc?.content || `
    <p>Privacy Policy content not found. Please create a record in the database with slug 'privacy-policy'.</p>
    <ul>
        <li>Collection: legals</li>
        <li>Slug: privacy-policy</li>
        <li>Title: Privacy Policy</li>
            ...
            <div className="prose prose-invert prose-lg max-w-none text-textMuted leading-relaxed">
              ...
              {!doc && (
                  <div className="mt-8 p-4 border border-yellow-500/30 bg-yellow-500/10 rounded-lg text-yellow-200 text-sm font-sans not-prose">
                      ...
                      <p>No record found in database with slug 'privacy-policy'. To make this dynamic:</p>
                      <ol className="list-decimal ml-4 mt-2 space-y-1">
                          <li>Create collection <code>legals</code>.</li>
                          <li>Add a record with <code>slug="privacy-policy"</code>.</li>
